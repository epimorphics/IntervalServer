...
@Path(UkCalURITemplate.DOC_STEM+UkCalURITemplate.HALF_SEGMENT+UkCalURITemplate.SET_EXT_PATTERN)
public class UkHalfDoc extends UkDoc {
	...
	public static Resource createResourceAndLabels(URI base, Model m, int year, int half) {
	...
	void addContainedIntervals() {
		...
		for (int quarter = 0; quarter < 2; quarter++) {
			...
			Resource r_quarter = UkQuarterDoc.createResourceAndLabels(base, model, year, i_qoy);
		...
		for (int month = 0; month < 6; month++) {
			...
			Resource r_month = UkMonthDoc.createResourceAndLabels(base, model, year, i_moy);	
	...
	void addContainingIntervals() {
		Resource r_year = UkYearDoc.createResourceAndLabels(base, model, year);
