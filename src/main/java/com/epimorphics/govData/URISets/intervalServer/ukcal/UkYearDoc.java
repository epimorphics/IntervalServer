...
@Path(UkCalURITemplate.DOC_STEM+UkCalURITemplate.YEAR_SEGMENT+UkCalURITemplate.SET_EXT_PATTERN)
public class UkYearDoc extends UkDoc {
	...
	protected void addContainedIntervals() {
		...
		for (int half = 0; half < 2; half++) {
			Resource r_half = UkHalfDoc.createResourceAndLabels(base, model, year, half+1);
			halves[half] = r_half;
		...
		for (int quarter = 0; quarter < 4; quarter++) {
			Resource r_quarter = UkQuarterDoc.createResourceAndLabels(base, model, year, quarter+1);
		...
		for (int month = 0; month < 12; month++) {
			Resource r_month = UkMonthDoc.createResourceAndLabels(base, model, year, month+1);		
