...
@Path(UkCalURITemplate.DOC_STEM+UkCalURITemplate.MINUTE_SEGMENT+UkCalURITemplate.SET_EXT_PATTERN)
public class UkMinuteDoc extends UkDoc {
	...
	public static Resource createResourceAndLabels(URI base, Model m, int year,	int moy, int dom, int hod, int moh) {
	...
	void addContainedIntervals() {
		...
		for (int sec = i_min_som; sec <= i_max_som; sec++) {
			Resource r_sec = UkSecDoc.createResourceAndLabels(base, model, year, month, day, hour, min, sec);
	...
	void addContainingIntervals() {
		...
		Resource r_thisHour    = UkHourDoc.createResourceAndLabels(base, model, year, month, day, hour);
