...
public class UkGovHalfDoc extends UkGovDoc {
	...
	public static Resource createResourceAndLabels(URI base, Model m, int year, int half) {
		...
		Resource r_half = m.createResource(s_halfURI, INTERVALS.BusinessHalf);
	...
	void addContainedIntervals() {
		...
		for (int quarter = 0; quarter < 2; quarter++) {
			...
			Resource r_quarter = UkGovQuarterDoc.createResourceAndLabels(base, model, year, i_qoy);
	...
	void addContainingIntervals() {
		Resource r_year = UkGovYearDoc.createResourceAndLabels(base, model, year);
